...
def post_detail(request, year, month, day, post):
    ...
    return render(request, 'post/detail.html', {'post':post, 'comment':comment, 'form':form, 'similar_posts':similar_posts})
...
def post_share(request, post_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messange = f"Read {post.title} at{post_url}\n\n" f"{cd['name']}\'s comments:{cd['comments']}"
            send_mail(subject, messange, 'your_account@gmail.com', [cd['to']])
            sent = True
...
def post_search(request):
    ...
    if 'query' in request.GET:
        ...
        if form.is_valid():
            ...
            ).filter(similarity_gt=0.3).order_by('-similarity')
